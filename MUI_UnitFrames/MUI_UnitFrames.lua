...
object "UnitFrames" : extends "Module" {
    ...
    _RegisterEvents = function(self)
        ...
        self.eventFrame:RegisterEventHandler("PLAYER_ENTERING_WORLD", function()
            -- Blizzard defers its PlayerFrame/TargetFrame reset by one tick, so
            -- defer Reanchor to the next tick to ensure it runs after Blizzard's
            -- reset completes.
            C_Timer.After(0, function()
                self.player:Reanchor()
                self.target:Reanchor()
            end)
            self.player:UpdateBars()
            self.combo:Update()
